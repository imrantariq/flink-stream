...
public class DynamicAlertFunction
  ...
  public void processElement(
    ...
    Rule ruleObj = ctx.getBroadcastState(Descriptors.rulesDescriptor).get(value.getId());

    if (noRuleAvailable(ruleObj)) {
      log.error("Rule with ID {} does not exist", value.getId());
      return;
    }


    if (ruleObj.getRuleState() == Rule.RuleState.ACTIVE) {
        if (ruleObj.getRuleType().equalsIgnoreCase("default")){
        DefaultRule rule = (DefaultRule) ruleObj;
      ...
      } else if (ruleObj.getRuleType().equalsIgnoreCase("hackrule")){
          HackRule rule = (HackRule) ruleObj;
          Long windowStartForEvent = rule.getWindowStartFor(currentEventTime);
        ...
        System.out.println("Not implemented rule type");
      }
  ...
  private void aggregateValuesInState(
      Long stateEventTime, SimpleAccumulator<BigDecimal> aggregator, Rule ruleObj) throws Exception {
    ...
    if (ruleObj.getRuleType().equalsIgnoreCase("default")){
      ...
      if (COUNT.equals(rule.getAggregateFieldName()) || COUNT_WITH_RESET.equals(rule.getAggregateFieldName())) {
        for (Transaction event : inWindow.stream()
                .filter(p -> p.getRuleType().equalsIgnoreCase("default")).collect(Collectors.toList())) {
          System.out.println("agg: " + event);
      ...
      if (COUNT.equals(rule.getAggregateFieldName())
        ...
        for (Transaction event : inWindow.stream()
                .filter(p -> p.getRuleType().equalsIgnoreCase("hackrule")).collect(Collectors.toList())) {
          System.out.println("agg: " + event);
          aggregator.add(BigDecimal.ONE);
        }
      } else {
  ...
  private void updateWidestWindowRule(Rule rule, BroadcastState<Integer, Rule> broadcastState)
    ...
    if (rule.getRuleType().equalsIgnoreCase("default") && widestWindowRule.getRuleType().equalsIgnoreCase("default")) {
      DefaultRule defaultRule = (DefaultRule) widestWindowRule;
      DefaultRule defaultRule2 = (DefaultRule) rule;

      if (defaultRule.getWindowMillis() < defaultRule2.getWindowMillis()) {
        broadcastState.put(WIDEST_RULE_KEY, defaultRule2);
      }
    } else if (rule.getRuleType().equalsIgnoreCase("hackrule") && widestWindowRule.getRuleType().equalsIgnoreCase("hackrule")) {
      HackRule ruleDefault = (HackRule) widestWindowRule;
      HackRule ruleDefault2 = (HackRule) rule;

      if (ruleDefault.getWindowMillis() < ruleDefault2.getWindowMillis()) {
        broadcastState.put(WIDEST_RULE_KEY, ruleDefault2);
      }
    }
  ...
  public void onTimer(final long timestamp, final OnTimerContext ctx, final Collector<Alert> out)
    ...
    if (widestWindowRule.getRuleType().equalsIgnoreCase("default")) {
      DefaultRule rule = (DefaultRule) widestWindowRule;

      Optional<Long> cleanupEventTimeWindow = Optional.ofNullable(rule).map(DefaultRule::getWindowMillis);
    ...
    } else if (widestWindowRule.getRuleType().equalsIgnoreCase("hackrule")) {
      HackRule rule = (HackRule) widestWindowRule;

      Optional<Long> cleanupEventTimeWindow = Optional.ofNullable(rule).map(HackRule::getWindowMillis);
